...
export default function ProfilePermissionTable() {
  ...
  const getAll = async (page: number) => {
    try {
      setLoading(true);
      const {data} = await api.get(`/profile-permissions?deleted=false&orderBy=createdAt&sort=desc&pageSize=10&pageNumber=${page}`, configApi());
  ...
  const destroy = async () => {
    try {
      setLoading(true);
      await api.delete(`/profile-permissions/${company.id}`, configApi());
  ...
  const getObj = (obj: any, action: string) => {
    ...
    if(action == "edit") {
      router.push(`/master-data/profile-permission/${obj.id}`);
    };
          ...
          <div className="min-w-[1102px] divide-y">
            <Table className="divide-y">
              ...
              <TableBody className="divide-y divide-gray-100 dark:divide-white/5">
                {pagination.data.map((x: TProfilePermission) => (
                  <TableRow key={x.id}>
