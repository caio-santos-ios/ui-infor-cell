...
import { TCustomer } from "@/types/master-data/customer/customer.type";
...
export default function CategoryForm({id}: TProp) {
  ...
  const { handleSubmit, reset, watch } = useForm<TEmployee>({
  ...
  const create: SubmitHandler<TCustomer> = async (body: TCustomer) => {
    try {
      ...
      router.push(`/master-data/suppliers/${result.data.id}`)
    } catch (error) {
  ...
  return (
    <>
      <ComponentCard title="Dados Gerais">
        <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-6 gap-2 container-form">  
          {/* 
      ...
      <Button type="submit" className="w-full xl:max-w-20 mt-2" size="sm">Salvar</Button>
    </>
  );
}