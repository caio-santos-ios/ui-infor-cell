...
import { TVariation, TVariationItem } from "@/types/product/variation/variation.type";
import Label from "@/components/form/Label";
import { Table, TableBody, TableCell, TableHeader, TableRow } from "@/components/ui/table";
import Button from "@/components/ui/button/Button";
import { MdCheck, MdOutlineQrCodeScanner } from "react-icons/md";
import { TPurchaseOrderItem, TVariationPurchaseOrderItem } from "@/types/purchase/purchase-order/purchase-order.type";
...
type TProp = { 
  ...
  productId: string;
  supplierId: string;
  sendVariations: (body: TPurchaseOrderItem) => void
};

export default function PurchaseOrderItemVariationForm({ id, productId, supplierId, sendVariations }: TProp) {
  ...
  const [serials, setSerial] = useState<TSerial[]>([{code: '', cost: 0, price: 0, hasAvailable: true}]);
  const [currentVariation, setCurrentVariation] = useState<number>(-1);
  
  const { register, control, reset, getValues, setValue, watch } = useForm<TPurchaseOrderItem>({
  ...
  const updateSerial = async () => {
    ...
    body.variations[currentVariation].serials = serials;
