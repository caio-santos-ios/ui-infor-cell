...
import VariationModalCreate from "./VariationModalCreate";
...
export default function VariationTable() {
  ...
  const getAll = async (page: number) => {
    try {
      setLoading(true);
      const {data} = await api.get(`/variations?deleted=false&orderBy=createdAt&sort=desc&pageSize=10&pageNumber=${page}`, configApi());
  ...
  const destroy = async () => {
    try {
      setLoading(true);
      await api.delete(`/variations/${company.id}`, configApi());
  ...
  const getObj = (obj: any, action: string) => {
    ...
    if(action == "edit") {
      router.push(`/product/variations/${obj.id}`);
    };
  ...
  return (
    <>
    {
      ...
      <>
                ...
                <TableBody className="divide-y divide-gray-100 dark:divide-white/5">
                  {pagination.data.map((x: any) => (
                    <TableRow key={x.id}>
                      ...
                      <TableCell className="px-5 py-4 sm:px-6 text-start text-gray-500 dark:text-gray-400">{x.active ? 'Ativo' : 'Inativo'}</TableCell>
        ...
        <ModalDelete confirm={destroy} isOpen={isOpen} closeModal={closeModal} title="Excluir Variação" />          
      </>
      :
      <NotData />
    }
    <VariationModalCreate />
    </>
  );
}